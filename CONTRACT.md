...
- `fetchQuickBattle({ restoreToken?, refit?, champion?, skin? })` → current pair + preloaded next pair + battle stats. Pairs are HMAC-signed stateless tokens (no row written on read); `refit` triggers the manual Bradley-Terry refit (guarded by `GAMES_ADMIN_SECRET` when set; reachable for cron via `GET /games/quick-battle?refit=…` which runs the loader). `champion` (a champion id, any casing) scopes the deal to that wardrobe - the "help settle" loop behind `/battle?champion=<id>` - and `skin` pins that skin into the first pair; the response's `scope` carries the resolved wardrobe and its ranking's verdict state, or `null` when the request did not resolve and the whole catalog is in play (`docs/participation-loop.md`)
- `submitBattleVote({ pairToken, winnerId, recent?, restoreToken?, mode?, champion? })` → appends the raw `battle_voted` event, applies the live Elo update (global + personal), burns the pair nonce, enforces rate limits, and returns feedback (delta, rank, agreement %, and in a scoped session the winner's place within its wardrobe) + the next pair, dealt from the scoped wardrobe when `champion` is given. The vote itself is the same signed pick whatever pool it came from
- `fetchSettleHub()` → `/settle`: every champion whose ranking is provisional or untouched, judged by the `answer.ts` bars from one aggregate voter query, with the bar its leader still misses. Anonymous, `noindex`
- `fetchPriceCheck({ restoreToken? })` / `submitPriceGuess({ tier, restoreToken? })` → Price Point (`/battle/price-point`; internal id `price-check`): five seeded skins a day, guess each one's RP tier. Prices come from the committed facts snapshot (`web/src/lib/games/data/skin-facts.json`, League Wiki SkinData + CommunityDragon rarity, refreshed by `web/scripts/snapshot-facts.mjs` — never fetched at runtime). The unanswered round's price never reaches the client; win = 3+ exact of 5
